...
const slug = (BUSINESS.name || "dudao").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "dudao";
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(buf as ArrayBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${slug}-cashup-${date}.xlsx"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (err) {
    return errorResponse(err, "build the export");
  }
}
