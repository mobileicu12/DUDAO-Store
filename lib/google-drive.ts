...
/**
 * Minimal Google Drive uploader for the daily backup.
 *
 ...
 * app can only ever see and manage files it created itself — it cannot read the
 * rest of the owner's Drive — so backups live in their own app-created folder.
 *
 * Everything here is plain HTTPS/REST (no SDK), so it works anywhere fetch does.
 */
...
export async function uploadTextToDrive(opts: {
  ...
  if (!up.ok) {
    throw new Error(`Drive upload failed (${up.status}).`);
  }
